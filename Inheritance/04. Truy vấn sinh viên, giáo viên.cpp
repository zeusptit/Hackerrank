...
class Teacher : public Person{
    ...
    public:
        ...
        Teacher(string id,string name, string bth, string adr,string clas,int slr,string cn) : Person(id, name, bth, adr, clas)
        {
            this->slr = slr;
            this->cn = cn;
        }
...
int main(){
    ...
    for(int i = 1 ; i <= n; i++)
    {
        ...
        if(id[0] == 'S')
        {
            ...
            int slr;cin >> slr;
            string cn; cin >> cn;
            Teacher t(id,name,bth,adr,clas,slr,cn);
            t.sdd();
            v2.pb(t);
        }
    }
    ...
    cout << "DANH SACH GIAO VIEN CO DIA CHI TAI " << temp << " : " << endl;
    for(int i = 0; i < v2.size(); i++)
        if(v2[i].getAdr() == temp)
            v2[i].out();
    cout << "DANH SACH SINH VIEN CO DIA CHI TAI " << temp << " : " << endl;
